...
# Only the leaves are stubbed, so a subshell anywhere on the way down to
# generate_ota_info fails this too
DEVICE_NAME="bluejay" INTERACTIVE_MODE="false" . src/main.sh

# release.yml sources the script too, so a trap the library installs would land
# on the workflow step's shell and replace whatever was there
if [[ "$(trap -p EXIT)" != "${caller_trap}" ]]; then
  echo "::error::Sourcing \`src/main.sh\` replaced the caller's EXIT trap."
  exit 1
fi
